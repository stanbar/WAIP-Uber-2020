...
public class TpAddressReader implements SpecializedObjectReader {

	public static final TpAddressReader INSTANCE = new TpAddressReader();

	public Object getAny(Properties aProps, String aValue,
			boolean aIsIndirected, Class aTargetClass) {
		if (aTargetClass != TpAddress.class) {
		...
		return new TpAddress((TpAddressPlan) getField(tokens[1],
				TpAddressPlan.class, TpAddressPlan.P_ADDRESS_PLAN_E164),
				tokens[0], tokens[2], (TpAddressPresentation) getField(
						tokens[3], TpAddressPresentation.class,
						TpAddressPresentation.P_ADDRESS_PRESENTATION_ALLOWED),
				(TpAddressScreening) getField(tokens[4],
						TpAddressScreening.class,
